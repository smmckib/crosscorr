...
%
function [lag, R_xy, P_xy, n,Sig_xy] = cross_corr(x, y, nlags, bad_flag)
%
% input:
%       x,y the data sets
...
% continue with this lag if there are data
    if (Ngoodx>0)
...
% estimate cov. and corr.
            R_xy(cnt) = z1'*z2/Ngood; % Equation 6.2.78
            P_xy(cnt) = R_xy(cnt)/(std_1*std_2);% Equation 6.2.82
            
% use matlab function to get P values
[R,P]= corrcoef(z1,z2);
            Sig_xy(cnt)=P(1,2);
